...
tf.app.flags.DEFINE_string('train_file_path', 'data/foody/train.p', 'training file')
tf.app.flags.DEFINE_string('test_file_path', 'data/foody/test.p', 'testing file')
tf.app.flags.DEFINE_string('embedding_file_path', 'data/foody/w2v.100.p', 'word embedding file')
...
class LSTM(object):
    ...
    def __init__(self, embedding_dim=100, batch_size=64, n_hidden=100, learning_rate=0.01,
        ...
        self.n_iter = n_iter
        self.type_ = type_
        self.word_id_mapping, self.w2v = cPickle.load(open(FLAGS.embedding_file_path, 'rb'))
    ...
    def run(self):
        ...
        target = tf.reduce_mean(tf.nn.embedding_lookup(self.word_embedding, self.target_words), 1, keepdims=True)
        ...
        with tf.Session() as sess:
            ...
            tr_x, tr_sen_len, tr_x_bw, tr_sen_len_bw, tr_y, tr_target_word = cPickle.load(
                open(FLAGS.train_file_path, 'rb')
            )
            te_x, te_sen_len, te_x_bw, te_sen_len_bw, te_y, te_target_word = cPickle.load(
                open(FLAGS.test_file_path, 'rb')
            )
